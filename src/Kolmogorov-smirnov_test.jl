function Kolmogorov_smirnov_test{T<:UnivariateDistribution}(dat::AbstractArray,d::T,xmax::Int64 = round(Int,1e5))
  alpha, xmin = params(d)
  data = sort(dat)
  n = float(length(data))
  if (data[end] > xmax)
  ...
  if (typeof(d) == con_powerlaw)
    act_cdf = [0:length(data)-1] / n
    thr_cdf = cdf(d,float(data))
  elseif (typeof(d) == dis_powerlaw)
    ...
    throw("Not implemented for type $(typeof(d))")
  end
